...
export const useTaskStore = create((set, get) => ({
  ...
  updateTask: (updatedTask) => {
    set((state) => {
      ...
      const newStorageKey = formatDate(updatedTask.date);
      ...
      if (oldStorageKey !== newStorageKey) {
        ...
        updatedTasks[newStorageKey].push({
          ...
          originalDate: updatedTask.isRepetitive ? undefined : oldStorageKey, // Only set originalDate if it's now a daily task
          repetitionEndDate:
            updatedTask.isRepetitive && updatedTask.repetitionEndDate
              ? formatDate(updatedTask.repetitionEndDate)
          ...
          task.id === updatedTask.id
            ? {
                ...
                originalDate: updatedTask.isRepetitive
                  ? undefined
                  : taskInStore.originalDate, // Preserve originalDate if it was already a daily task with one
                repetitionEndDate:
                  updatedTask.isRepetitive && updatedTask.repetitionEndDate
                    ? formatDate(updatedTask.repetitionEndDate)
                    : undefined, // Format end date if repetitive, clear if not
    ...
    set((state) => {
      ...
      if (originalDateKey !== newDateKey) {
        ...
        const rescheduledTask = {
          ...
          // If it was a daily task, originalDueDate tracks its previous date.
          ...
          // If a repetitive task is rescheduled, it effectively becomes a daily task instance
          isRepetitive: false,
